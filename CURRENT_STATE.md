...
- Each channel now targets at most eight occupied posts so Buffer Free retains two recovery slots. `scheduled`, `error` and `sending` posts all count against that shared budget. A legacy over-budget queue is reduced by removing only the farthest-future managed Daily Thread posts; unrelated manual, failed and sending posts are preserved.
